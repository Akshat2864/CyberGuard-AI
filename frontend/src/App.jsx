  ...
  const handleOAuth = async (provider) => {
    ...
    try {
      ...
      if(response.ok && data.url) {
        ...
        setAuthError(data.error || `Failed to initiate ${provider} handshake.`);
        setIsAuthenticating(false);
      }
    } catch(err) {
      setAuthError('Grid link timeout. Handshake failed.');
          ...
          {isLogin && !isForgotPassword && (
            <>
              <div style={{ display: 'flex', gap: '1rem', marginBottom: '0.5rem' }}>
                <button type="button" onClick={() => handleOAuth('github')} className="card" style={{ flex: 1, padding: '0.8rem', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '0.5rem', cursor: 'pointer', border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(255,255,255,0.03)' }}>
                ...
                <button type="button" onClick={() => handleOAuth('google')} className="card" style={{ flex: 1, padding: '0.8rem', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '0.5rem', cursor: 'pointer', border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(255,255,255,0.03)' }}>
                  <svg width="18" height="18" viewBox="0 0 24 24" fill="white"><path d="M12.48 10.92v3.28h7.84c-.24 1.84-.853 3.187-1.787 4.133-1.147 1.147-2.933 2.4-6.053 2.4-4.827 0-8.6-3.893-8.6-8.72s3.773-8.72 8.6-8.72c2.6 0 4.507 1.027 5.907 2.347l2.307-2.307C18.747 1.44 16.133 0 12.48 0 5.867 0 .307 5.387.307 12s5.56 12 12.173 12c3.573 0 6.267-1.173 8.373-3.36 2.16-2.16 2.84-5.213 2.84-7.667 0-.76-.053-1.467-.173-2.053H12.48z"/></svg>
                  Google
                </button>
              </div>
...
const OAuthCallback = () => {
  ...
  useEffect(() => {
    ...
    const token = hashParams.get('access_token') || queryParams.get('code');

    if (token) {
      setTimeout(() => {
        login({ name: 'OAuth Operator', id: 'oauth-session-' + Math.random().toString(36).substr(2, 9) });
        navigate('/dashboard');
      }, 1000);
    } else {
      navigate('/login');
  ...
  return (
    <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: '75vh', flexDirection: 'column', gap: '1rem' }}>
      <motion.div animate={{ rotate: 360, color: 'var(--accent-blue)' }} transition={{ duration: 1.5, repeat: Infinity, ease: "linear" }}>
        <KeyRound size={48} />
      </motion.div>
      <h2 style={{ fontSize: '1.5rem' }}>Verifying OAuth Handshake...</h2>
    </div>
  );
};
